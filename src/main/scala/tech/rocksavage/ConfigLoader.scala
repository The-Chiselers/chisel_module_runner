...
/** An object responsible for loading configuration classes based on the
  * provided command-line arguments.
  */
object ConfigLoader {

    /** Loads configurations based on the subcommand provided in the
    ...
    def loadConfigs(conf: tech.rocksavage.args.Conf): Map[String, Any] = {
        conf.subcommand match {
            case Some(conf.verilog) =>
                loadConfig(
                  conf.verilog.configClass.toOption
                ) // Load config for Verilog generation
            case Some(conf.synth) =>
                loadConfig(
                  conf.synth.configClass.toOption
                ) // Load config for synthesis
            case Some(conf.sta) =>
                loadConfig(
                  conf.sta.configClass.toOption
                ) // Load config for static timing analysis
            case Some(conf.docs) =>
                loadConfig(
                  conf.docs.configClass.toOption
                ) // Load config for static timing analysis
            case _ => {
    ...
    /** Loads the configuration class dynamically using reflection.
      ...
      *   The classpath of the configuration class.
      * @return
      *   A map of configuration names to their corresponding parameters.
      */
    private def loadConfig(
        configClassPath: Option[String]
    ): Map[String, Any] = {
        configClassPath match {
            case Some(path) =>
                val configClass = Class
                    .forName(path)
                    .asSubclass(classOf[ModuleConfig]) // Load the class
                val configInstance = configClass
