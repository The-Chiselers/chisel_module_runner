...
/** A class that defines and parses command-line arguments using Scallop. It
  * supports two subcommands: `verilog` and `synth`, each with their own
  * options.
  *
  * @param arguments
...
class Conf(arguments: Seq[String]) extends ScallopConf(arguments) {
    ...
    object verilog extends Subcommand("verilog") {
        val mode = opt[String](
          ...
          descr =
              "Mode of operation: 'print' to print Verilog to console, 'write' to write to a file."
        )
        ...
        val configClass = opt[String](
          default = None,
          descr =
              "Classpath to the configuration class implementing ModuleConfig.",
          required = true
        )
    }
    ...
    /** Subcommand for running static timing analysis (STA).
      */
    object docs extends Subcommand("docs") {
        val module = opt[String](
          required = true,
          descr = "The name of the module to synthesize."
        )
    ...
    addSubcommand(docs)    // Register the docs subcommand
