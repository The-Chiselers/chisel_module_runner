...
/** An object responsible for generating Verilog code based on the provided
  * configurations.
  */
object VerilogGenerator {
  ...
  def generate(conf: Conf, defaultConfigs: Map[String, Any]): Unit = {
    ...
    defaultConfigs.foreach { case (name, params) =>
      ...
      val verilogString = genVerilogFromModuleName(
        verilogConf.module(),
        params
      ) // Generate Verilog
      ...
      verilogConf.mode() match {
        ...
        case "write" =>
          val filename =
            s"./out/verilog/${verilogConf.module()}_$name.sv" // Define the output filename

          // Create the output directory if it doesn't exist
          val dir = new File("./out/verilog")
